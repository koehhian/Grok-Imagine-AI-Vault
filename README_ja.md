...
-   **スマートな ID 抽出**：画像をドラッグ＆ドロップしたり、複雑なデータを貼り付けたりしても、元の Grok ID を自動的に抽出します。**重複したリンクは自動的にフィルタリング**され、ライブラリをクリーンに保ちます。
-   **自動ローカルバックアップ**：サーバーを起動すると、リモートの画像サムネイルを自動的にローカルキャッシュに保存し、リンク切れを防ぎます。
-   **システムへの一括ダウンロード**：保存したすべての作品を、コンピュータの**「システムデフォルトのダウンロードフォルダ」**に直接ダウンロードして簡単にアクセスできます。
